...
def evaluate(output_with_label, data_tag):
    ...
    for index, device in enumerate(devices_order):
        ...
        for k in range(1, 6):
            ...
            device_dict["recall@%d" % k] = recall(output_zip_label, device, k, data_tag)
            # device_dict["nDCG@%d" % k] = nDCG(output_zip_label, k, data_tag)
        result[device] = device_dict
    return result

def recall(output_zip_label, device, k, data_tag):
    sample_cnt = 0
    tp, fp, fn = 0, 0, 0
    ...
    r = 1.0 * tp / (tp + fn)
    print("%s %s recall@%d: %.4f" % (data_tag, device, k, r))
    return r
